...
export class CategoriaService {
	...
	removerCategoria(categoria: ICategoria) {
		return this.httpClient
			.delete<ICategoria>(`http://localhost:8080/categoria/${categoria.id}`)
			.toPromise();
	}
	...
	getFullCategoria(): Promise<ICategoria[]> {
		return this.httpClient
			.get<ICategoria[]>('http://localhost:8080/categoria/full-categoria').toPromise();
	}
